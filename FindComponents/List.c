...
// deleteNext()
// Deletes element after cursor from this List if it exists.
void deleteNext(List L){
   if(L == NULL) {
      printf("List Error: deleteNext() called on NULL List\n");
      exit(1);
   }else if(L->back == NULL) {
      printf("List Error: deleteNext() called on empty List\n");
      exit(1);
   }else if(L->cursor == NULL) {
      printf("List Error: deleteNext() called on NULL cursor\n");
      exit(1);
   }else if(L->cursor->next == L->back){
      deleteBack(L);
   }else {
      Node temp = L->cursor->next;
      L->cursor->next = temp->next;
      temp->next->prev = L->cursor;
      L->length--;
      freeNode(&temp);
   }
}
...
// deletePrev()
// Deletes element before cursor from this List if it exists.
void deletePrev(List L){
   if(L == NULL) {
      printf("List Error: deletePrev() called on NULL List\n");
      exit(1);
   }else if(L->back == NULL) {
      printf("List Error: deletePrev() called on empty List\n");
      exit(1);
   }else if(L->cursor == NULL) {
      printf("List Error: deletePrev() called on NULL cursor\n");
      exit(1);
   }else if(L->cursor->prev == L->front){
      deleteFront(L);
   }else {
      Node temp = L->cursor->prev;
      temp->prev->next = L->cursor;
